...
### 4. Export Policy for Sim2Sim

Export ONNX and TorchScript policy artifacts from a trained checkpoint:

```bash
uv run export-policy Mjlab-Gensong-EasyWheel-Flat \
  --checkpoint-file logs/rsl_rl/wf_gensong_easy_wheel_only/<run_dir>/model_300.pt \
  --output-dir export/policy
```

To fetch a checkpoint from W&B and export directly:

```bash
uv run export-policy Mjlab-Gensong-EasyWheel-Flat \
  --wandb-run-path your-org/mjlab/run-id \
  --wandb-checkpoint-name model_300.pt \
  --output-dir export/policy
```
