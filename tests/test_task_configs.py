...
def test_training_mode_observation_corruption_enabled(all_task_ids: list[str]) -> None:
  ...
  for task_id in all_task_ids:
    ...
    expected = task_id not in _TRAINING_CORRUPTION_DISABLED_TASKS
    assert policy_obs.enable_corruption is expected, (
      f"Training task {task_id} has enable_corruption={policy_obs.enable_corruption}, "
      f"expected {expected}"
    )
